...
long long vocab_size = 0, vocab_max_size = 1000, train_words = 1000000;
...
{
   unsigned long long sentence[MAX_SEN + 1], rand_gen = 1, iteration, train_word_count = 0, l1;
      ...
      */
      while (1)
      {
         ...
         if (iteration % 300000 == 0)
         {
            ...
            cout << g << "         " << f << "       " << Weight_emb[1000][100] << endl;
...
void see_word()
{
   ...
   for (layer = 0; layer < embed_size; layer++) Weight_emb[i][layer] /= norm[i];
   ...
   {
      for (layer = 0; layer < embed_size; layer++) word_vec[layer] = 0;
      for (layer = 0; layer < embed_size; layer++) word_vec[layer] = Weight_emb[j][layer];
...
void save_all()
{
   ...
   if (fp)
   {
    ...
    for (int a = 0; a < vocab_size; a++) fwrite(&Weight_emb[a], sizeof(float) * embed_size, 1, fp);
    fclose(fp);
      /*
...
void load()
{
   ...
   for (int a = 0; a < vocab_size; a++) fread(&Weight_emb[a], sizeof(float) * embed_size, 1, fs);
